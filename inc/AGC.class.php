<?php
/**
 * Advanced Graphing Class - PHP4 backport
 *
 * This is a quick (and probably dirty) backport of Zack Bloom's
 * Advanced Graphing Class for PHP 4.3+
 *
 * It should probably still work with PHP5 but that's untested.
 *
 * IMPORTANT: do not report bugs to Zack Bloom!
 *
 * @Changes:
 *
 * - Renamed the clas to AGC to avoid function <-> constructor clash
 * - Removed all PHP5 specific OO syntax
 * - Replaced the class static counter with a global var
 * - Using register_shutdown_function to simulate the destructors
 * - Replaced arial.ttf with Vera.ttf because of unclear license
 * - Removed error reporting line
 * - Removed JavaScript graph generation because I don't need it
 *
 * @Todo
 *
 * - make sure everything still works
 * - add PHPdoc comments using the info from the README
 *
 * This backport was done by Andreas Gohr <andi@splitbrain.org>, all
 * original work was done by Zack Bloom <zackbloom@gmail.com>
 *
 * @author  Zack Bloom <zackbloom@gmail.com>
 * @author  Andreas Gohr <andi@splitbrain.org>
 * @license Public Domain, commercial tools need to give free license to Zack Bloom
 *
 * Original copyright notice and license info follows:
 *
 * ----------------------------------------------------------------------------
 ...
 *
 * Their are many examples of how to do almost anything you could ever want
 * to do (using this program, can't help you win the lottery) in the readme
 * file. The example page can be a big help for those who lack the attention
 * span to read the readme, all code will work on most setups verbatim
 *
 ...
 *
 * Licence
 * You can do what ever you want with the code, copy all or part, impress
 * girlfriends with your l33t programming skillz, say you wrote it, whatever.
 *
 * But I must say it is much more gratifing to write the code yourself (unless
 * it actually takes effort like the AA lines or arrow heads that I stole
 * functions for, then steal away)
 *
 * You don't even have to give me credit, just send me a link to any thing
 * cool you make, zackbloom@gmail.com.
 *
 * If you develop any for-profit applications, the only requirement is that
 * I get a free account or copy of the program.
 * ----------------------------------------------------------------------------
 */
...
class AGC {
    ...
    function AGC($x_width=400,$x_height=200,$xScale=10,$yScale=6){
        $this->props = array();
        $this->xPts = array();
        $this->yPts = array();
        $this->iVars[0] = array();
        ...
        $this->setProp('font',dirname(__FILE__).'/Vera.ttf');
        $this->setProp('keyfont',dirname(__FILE__).'/Vera.ttf');
    ...
    function graph(){ //Allows internalGraph to return false if it needs to restart
        do
    ...
    function indSort($e){
        ...
        usort($tiVars,array("AGC","actSort"));
    ...
    function drawPie($ex){
        ...
        for($i=1;$i<count($sortkeys)+1;$i++)
            if($sortkeys[$i-1]<90 && $sortkeys[$i]>90) // Make sure the one that actually crosses 90 is last
                $sortkeys[$i] = 90;
        uasort($sortkeys,array('AGC','piesort'));
        ...
        for($i=0;$i<count($da['data']);$i++){ //Draw keys
            if($this->getProp("showval",true)){
                $text = ($this->getProp("useval",false)?$da['data'][$i]:round($da['part'][$i]*100,0).'%');
            }else{
                $text = '';
            }
            $size = imagettfbbox($this->getProp("textsize",8),$this->getProp("textAngle",0),$this->getProp("font","Vera.ttf"),$text);
            $avang = ($da['ansum'][$i-1]+$da['ansum'][$i])/2;
            imagettftext($this->img,$this->getProp("textsize",8),$this->getProp("textAngle",0),cos(deg2rad($avang))*$w/2+$cx+($avang<90||$avang>270?$this->getProp("numspace",5):-$size[2]-$this->getProp("numspace",5)),sin(deg2rad($avang))*$h/2+$cy+($avang<180&&$avang>0?-$size[5]+$ang:$size[5]),imagecolorallocate($this->img,0,0,0),$this->getProp("font","Vera.ttf"),$text);
        }
    }
    ...
    function internalGraph(){
        ...
        if($this->getProp("type","line")!="pie"){
            if($this->getProp("showhorizscale",true)){
                ...
                for($i=0;$i<$this->getProp("xincpts")+1;$i++){//Create horiz scale
                    ...
                    $size = imagettfbbox($this->getProp("textsize",8),$this->getProp("textAngle",0),$this->getProp("font","Vera.ttf"),$text);
                    imagettftext($this->img,$this->getProp("textsize",8),$this->getProp("textAngle",0),round(($i*($this->width/$this->getProp("xincpts")))-($i!=$this->getProp("xincpts")?($i!=0?(.5*$size[2]):0):$size[2]),0)-($this->getProp("type","line")=="bar"?.5*($this->width/count($this->xPts[0])):0),$this->height+abs($size[5])+3,$grid,$this->getProp("font","Vera.ttf"),$text);
                }
            }
            if($this->getProp("showvertscale",true)){
                for($i=0;$i<$this->getProp("yincpts")+1;$i++){//Create vert scale
                    ...
                    $size = imagettfbbox($this->getProp("textsize",8),$this->getProp("textAngle",0),$this->getProp("font","Vera.ttf"),$text);
                    imagettftext($this->img,$this->getProp("textsize",8),$this->getProp("textAngle",0),$this->width+3,round((($this->getProp("yincpts")-$i)*($this->height/$this->getProp("yincpts")))-($text!=$this->yMax?($text!=$this->yMin?(.5*$size[5]):0):$size[5]),0),$grid,$this->getProp("font","Vera.ttf"),$text);
                }
            }
        }
        ...
        $sX = ImageSX($this->img);
        $sY = ImageSY($this->img);
        $font = $this->getProp("font","Vera.ttf");
        ...
        if($this->getProp("showkey",false)){
            ...
            $font = $this->getProp("keyfont","Vera.ttf");
            $indhei = 0;
            $indwid = 0;
    ...
    function createGraph($data,$w=400,$h=200,$prop="",$random=false){
        ...
        $graph->graph();
        $name = $graph->getProp("tempfolder","")."graph".substr(basename($_SERVER['PHP_SELF']),0,3).$AGC__counter.($random==true?rand(1,100000):"").'.png';
        ...
        if($random && $graph->getProp("delold",true)){
            ...
            $f[]=$name;
            file_put_contents($fname,join(array_filter($f,"AGC::trimfilter"),"\n"));
        }
        unset($graph);
        return $name;
    }
